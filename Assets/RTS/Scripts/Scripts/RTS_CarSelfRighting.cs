...
public class RTS_CarSelfRighting : MonoBehaviour{
	// Automatically put the car the right way up, if it has come to rest upside-down.
	[SerializeField] private float Delay_WaitTime = 3f;           // time to wait before self righting
	...
	private void Update(){
		...
		if (Time.time > m_LastOkTime + Delay_WaitTime){
			RightCar__AI_extreme();
		}
	}

	// put the car back the right way up:
	private void RightCar__AI_extreme()	{
