...
public class CarController : MonoBehaviour{
	...
	private bool CheckIfSkidSoundPlaying(){
		for (int i = 0; i < 4; i++)	{
	...
	private void Start(){
		if (playerCar){
			if (RTS_PlayerPrefs.GetAudio() == "OFF") {
				isAudioMuted = true;
			} else {
				isAudioMuted = false;
			}
			...
			if(PlayerPrefs.GetInt("RTS_ModeSelection") == 0) {
				isFire = true;
            }



			manual = RTS_PlayerPrefs.playableVehicles.manual;
			GameObject lobbyImage = GameObject.Find ("Lobby Loading Image");
			if(lobbyImage != null)	lobbyImage.SetActive(false);
			SetupUpdated__VehicleStats ();
		...
		switch (RTS_PlayerPrefs.playableVehicles.speedometerType[RTS_PlayerPrefs.raceData.vehicleNumber]) {
		case RTS_DistanceUnits.SpeedType__CarRacing.KilometerPerHour:
			distanceMetricText.text = "KPH";
			break;
		case RTS_DistanceUnits.SpeedType__CarRacing.MilesPerHour:
	...
	void Check__Current__Gear(){
		...
		if (!manual) { 
			...
			if (canShift) {
				if (Input.GetKeyDown (RTS_PlayerPrefs.inputData.shiftUp) || Input.GetKeyDown (RTS_PlayerPrefs.inputData.shiftUpJoystick)) {
					canShift = false;
					ShiftUp ();			
				}
				if (Input.GetKeyDown (RTS_PlayerPrefs.inputData.shiftDown) || Input.GetKeyDown (RTS_PlayerPrefs.inputData.shiftDownJoystick)) {
					canShift = false;
					ShiftDown ();
				}
			}
			else {
				if (Input.GetKeyUp (RTS_PlayerPrefs.inputData.shiftUp) || Input.GetKeyUp (RTS_PlayerPrefs.inputData.shiftUpJoystick)) {
					canShift = true;		
				}
				if (Input.GetKeyUp (RTS_PlayerPrefs.inputData.shiftDown) || Input.GetKeyUp (RTS_PlayerPrefs.inputData.shiftDownJoystick)) {
					canShift = true;
				}
			}
		}
...
//				if (RTS_PlayerPrefs.inputData.useMobileController) {
//					
//				} else {
//					
//				}
		...
		var revsRangeMin = ULerp(0f, revRangeBoundary, Cal__CurveFactor(gearNumFactor));
	...
	private static float Cal__CurveFactor(float factor){
	...
	public void Move(float steering, float accel, float footbrake, float handbrake){
		...
		if(overrideAcceleration){
			...
			SetDriveVehicle(accel, footbrake);
			return;
		}
		SteerTracHelp();
		SetDriveVehicle(accel, footbrake);
		...
		CalGearWithSpeed ();
		Check__Current__Gear();
		UpdateGameUI ();
		AddCMSDownForce();
		CheckForWheelMovment();
		Traction__Setup__Control();
	}

	private void SetDriveVehicle(float accel, float footbrake){
		if (playerCar && manual) {
	...
	private void CheckForWheelMovment(){
	...
	private void Traction__Setup__Control(){
		WheelHit wheelHit;
		switch (m_CarDriveType)
		{
		case CarDriveType.FourWheelDrive:
			...
			for (int i = 0; i < 4; i++){
				...
				Adjust__TorqueFactor(wheelHit.forwardSlip);
			}
			break;
	...
	private void Adjust__TorqueFactor(float forwardSlip){
	...
	void SetupUpdated__VehicleStats(){
		vehicleNumber = RTS_PlayerPrefs.GetVehicleNumber ();
	...
	IEnumerator fire() {
		...
		//var randTime = Random.Range(8,10);

		//yield return new WaitForSeconds(randTime);
		//var fire = Instantiate(firePrefab, transform);
		//fire.transform.position = transform.position;
		//fire.GetComponent<Rigidbody>().AddForce(fire.transform.forward * projectileSpeed, ForceMode.Impulse);
		//print("fire");
		//isFire = false;
