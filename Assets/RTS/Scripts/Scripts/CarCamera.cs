...
public class CarCamera : MonoBehaviour {
	...
	void Start(){
		useRearViewMirror = RTS_PlayerPrefs.playableVehicles.rearMirror;
		useSideViewMirrors = RTS_PlayerPrefs.playableVehicles.sideMirrors;
		useReflectionProbe = RTS_PlayerPrefs.playableVehicles.reflectionProbe;
	...
	void Update(){
		if (car != null) {
			if (cameraType == CameraType.CarCamera) {				
				...
				Enable__InteriorComponents ();
				carCamera.gameObject.SetActive (false);
			}
			if (Input.GetKeyDown (RTS_PlayerPrefs.inputData.lookBack) || Input.GetKeyDown (RTS_PlayerPrefs.inputData.lookBackJoystick) ) {
				LookBackCamera ();
			} else if( Input.GetKeyUp (RTS_PlayerPrefs.inputData.lookBack) || Input.GetKeyUp (RTS_PlayerPrefs.inputData.lookBackJoystick) ){
			...
			if (Input.GetKeyDown (RTS_PlayerPrefs.inputData.cameraSwitchKey) || Input.GetKeyDown (RTS_PlayerPrefs.inputData.cameraSwitchJoystick)) {
				CycleCamera ();
			}
			if (Input.GetKeyDown (RTS_PlayerPrefs.inputData.nitroKey) || Input.GetKeyDown (RTS_PlayerPrefs.inputData.nitroJoystick)) {
				carController.NitroOn ();
			}
			if(Input.GetKeyUp (RTS_PlayerPrefs.inputData.nitroKey) || Input.GetKeyUp (RTS_PlayerPrefs.inputData.nitroJoystick)){
	...
	void Disable__InteriorComponents ( ) {
	...
	void Enable__InteriorComponents ( ) {
		if (useRearViewMirror) {
    ...
    void FixedUpdate(){
		...
		if (cameraType == CameraType.CarCamera) {
			if (car && !mobile) {
				...
				var localVelocity = car.InverseTransformDirection (carBody.velocity);
				if (localVelocity.z < -0.5f && FYP.ExtremeRacingRush.CrossPlatformInput.PlatformSpecific.CrossPlatformInputManager.GetAxis ("Vertical") == -1) {
					rotationVector.y = car.eulerAngles.y + 180f;
				} else {
					rotationVector.y = car.eulerAngles.y;
				}
