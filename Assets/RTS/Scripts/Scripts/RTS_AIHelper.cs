...
public class RTS_AIHelper : MonoBehaviour {

	public enum ProgressStyle__AI { SmoothAlongRoute, PointToPoint,	}
	...
	//References
	public WaypointCircuit circuit;		// A route of waypoints used for pathfinding
	public Transform target;														// Current waypoint to drive toward
	public RTS_AIDetection detection;												// AI sensor system used for obstacle detection and avoidance
	private RTS_SceneManager sceneManager;
	...
	public ProgressStyle__AI progressStyle = ProgressStyle__AI.SmoothAlongRoute;	// whether to update the position smoothly along the route (good for curved paths) or just when we reach each waypoint.
	...
	[Header("Teleport to RTS_Waypoint")]
	...
	private void Awake(){
		...
		//carControllerRef.rbody.AddForce (Vector3.forward * 50000, ForceMode.Impulse );
	}

	void Start(){
		defaultSteerSensitivity = steerSensitivity;
		sceneManager = GameObject.Find ("Scene Manager").GetComponent<RTS_SceneManager>();
		circuit = GameObject.Find("Scene Manager").GetComponent<WaypointCircuit>();
		if (target == null)
			target = new GameObject(name + " RTS_Waypoint Target").transform;
		progressDistance = 0;
		progressNum = 0;
		if (progressStyle == ProgressStyle__AI.PointToPoint){
	...
	void Update () {
		...
		if (progressStyle == ProgressStyle__AI.SmoothAlongRoute){
		...
		if (sceneManager.countUp) ObstacleDetection__AI ();
		if (sceneManager.countUp) StuckCheck__AI ();
	}

	void ObstacleDetection__AI(){
	...
	private void FixedUpdate() {
		// use handbrake to stop	carControllerRef.Move(0, 0, -1f, 1f);
	...
	public void ResetTimer__AI (){
		timer = 0;
	}

	public void AdjustAIWaypointSpeedFactor__AI (float newSpeed) {
